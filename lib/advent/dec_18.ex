defmodule Advent.Dec18 do
  ...
  def first_recover(input \\ @default_input) do
    program =
      parse(input)
      |> Enum.with_index
    ...
    execute_until_recover(state)
  end

  defp execute_until_recover(%{recovered: result}) when not is_nil(result), do: result
  defp execute_until_recover(state) do
    state
    |> execute_one_line
    |> execute_until_recover
  end
  ...
  defp execute_one_line(state, {:rcv, [val]}) do
    val = eval(state.registers, val)
    %{
      state |
      recovered: if(val == 0, do: state.recovered, else: state.last_sound),
      current: state.current + 1
    }
  end
